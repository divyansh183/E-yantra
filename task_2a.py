...
def detect_ArUco_details(image):
    ...
    * ArUco_corners = 
    ...
    # Detect ArUco markers in the input image array
    corners, ids, _ = aruco.detectMarkers(image_array, aruco_dict, parameters=parameters)
    if ids is not None:
        for i in range(len(ids)):
            marker_id = ids[i][0]
            corner_points = corners[i][0]  # Extract the corner points of the marker
            center_x = int(np.mean(corner_points[:, 0]))
            center_y = int(np.mean(corner_points[:, 1]))
        
            # Calculate the orientation of the marker with respect to the vertical
            orientation = np.arctan2(corner_points[2][1] - corner_points[0][1], corner_points[2][0] - corner_points[0][0])
            center_x = int(center_x)
            center_y = int(center_y)
            
            ArUco_corners[marker_id] = np.round(corner_points).astype(int)
            
            ArUco_details_dict[marker_id] = [[center_x,center_y],math.degrees(orientation)]
